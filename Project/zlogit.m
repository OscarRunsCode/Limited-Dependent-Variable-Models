...
function [beta,vc]=zlogit(data,dep,ind,beta); 
%clc
...
fprintf('\n');
fprintf('Logit Model - The dependent variable is: %s\n',dep);
...
logl0=zlogit_logl(beta);               % LogL at starting values.
...
% Begin BHHH iterations.
     ...
     [d,vc,md]=zlogit_bhhh(beta);                % Compute BHHH update.
...
% Choose largest step that increases function value.
     ...
     deltf = -1;
          ...
          logl=zlogit_logl(gamma);           % Evaluate Logit logL.
